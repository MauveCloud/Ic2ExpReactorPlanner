...
public class AutomationSimulator extends SwingWorker<Void, String> {
    ...
    protected Void doInBackground() throws Exception {
        ...
        try {
                        ...
                        if (component != null) {
                            if (component.getMaxHeat() > 1) {
                                if (component.automationThreshold > component.getInitialHeat() && component.getCurrentHeat() >= component.automationThreshold) {
                                    ...
                                    publish(String.format(_("R%dC%d:+Replaced at %,d seconds."), row, col, reactorTicks));
            ...
            if (reactor.getCurrentHeat() <= reactor.getMaxHeat()) {
                publish(String.format(_("Reactor ran for %,d seconds without exploding.\n"), reactorTicks));
                publish(String.format(_("Reactor was active for a total of %,d seconds (%,d to %,d seconds at a time).\n"), activeTime, minActiveTime, maxActiveTime));
                publish(String.format(_("Reactor was inactive for a total of %,d seconds.\n"), inactiveTime));
                publish(String.format(_("Components replaced:\n%s"), replacedItems.toString()));
                ...
                if (reactor.getCurrentHeat() > 0.0) {
                    publish(String.format(_("Reactor remained at %,.2f heat after cycle.\n"), reactor.getCurrentHeat()));
                }
                for (int row = 0; row < 6; row++) {
                    for (int col = 0; col < 9; col++) {
                        ReactorComponent component = reactor.getComponentAt(row, col);
                        if (component != null && !component.isBroken()) {
                            if (component.getCurrentHeat() > 0.0) {
                                ...
                                publish(String.format(_("R%dC%d:+Had %,.2f heat left after cycle."), row, col, component.getCurrentHeat()));
                            }
                        }
                    }
                }
