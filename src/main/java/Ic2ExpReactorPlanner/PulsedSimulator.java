...
public class PulsedSimulator extends SwingWorker<Void, String> {
    ...
    protected Void doInBackground() throws Exception {
        ...
        try {
            ...
            if (totalRodCount == 0) {
                publish(_("No fuel rods found! Simulating a pulsed cycle makes no sense!"));
                return null;
            }
                ...
                for (int row = 0; row < 6; row++) {
                    for (int col = 0; col < 9; col++) {
                        ...
                        if (component != null && component.isBroken() && !alreadyBroken[row][col] && !component.getClass().getName().contains("FuelRod")) {
                            ...
                            publish(String.format(_("R%dC%d:%s+Broke after %,d seconds."), row, col, component.toString(), reactorTicks));
                        }
                    }
                }
            ...
            if (reactor.getCurrentHeat() <= reactor.getMaxHeat()) {
                publish(String.format(_("Cycle complete after %,d seconds.\n"), reactorTicks));
                if (reactorTicks > 0) {
                    if (reactor.isFluid()) {
